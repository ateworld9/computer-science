...
async function main() {
  ...
  str = str.slice(0, -1)
  const arr = str.split('\n')
  const groups = arr[1].trim().split(' ')
    .map((el, i) => [+el, i + 1]).sort((a, b) => a[0] - b[0])
  const audiences = arr[2].trim().split(' ')
    .map((el, i) => [+el, i + 1]).sort((a, b) => a[0] - b[0])
  let audNum = 0
  ...
  try {
    await fs.writeFile('output.txt', `${counter}\n${answer.slice(1).join(' ')}`)
  }
  catch (err) {
    console.error(err.message)
  }
