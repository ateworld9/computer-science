...
function sameKeySums(pairs) {
  const map = new Map()
  pairs.forEach((pair) => {
    if (pair[0] === 94200950903412363n)
      console.log(pair)

    if (map.has(pair[0]))
      map.set(pair[0], map.get(pair[0]) + pair[1])
    else
      map.set(pair[0], pair[1])
  })
  console.log(123)
  const tuples = [...map.entries()].sort((a, b) => a[0] - b[0])
  console.log(1233)
  for (let i = 0; i < tuples.length; i++)
    tuples[i] = tuples[i].join(' ')
...
async function main() {
  ...
  const arrs = str.split('\n').slice(1).map(el => el.split(' ').map(BigInt))
