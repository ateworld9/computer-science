...
const calcMaxDensity = pipe(sortRowsByDensity, first, getDensityCell);

const calcRowsProportionToMax = (rows) => (max) =>
  rows.map(pipe(getDensityCell, (densityCell) => proportion(max, densityCell)));

const appendProportionCell = (rows) => (proportions) =>
  rows.map((row, index) => appendCell(row, proportions[index]));

const appendTableProportionCol = (rows) =>
  pipe(
    calcMaxDensity,
...
const getDataset = pipe(readFile, toLines, parseTableLines);

const main = pipe(getDataset, appendTableProportionCol, renderTable);
